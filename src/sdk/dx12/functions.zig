...
pub const Functions = struct {
    ...
    pub fn find() !Self {
        ...
        const dxgi_module = os.Module.getLocal("dxgi.dll") catch |err| {
            misc.error_context.append("Failed to get local module: dxgi.dll", .{});
            return err;
        };

        const create_dxgi_factory_address = dxgi_module.getProcedureAddress("CreateDXGIFactory") catch |err| {
            misc.error_context.append("Failed to get procedure address of: CreateDXGIFactory", .{});
            return err;
        };
        ...
        const d3d12_module = os.Module.getLocal("d3d12.dll") catch |err| {
            misc.error_context.append("Failed to get local module: d3d12.dll", .{});
            return err;
        };

        const create_device_address = d3d12_module.getProcedureAddress("D3D12CreateDevice") catch |err| {
            misc.error_context.append("Failed to get procedure address of: D3D12CreateDevice", .{});
            return err;
        };
